...
// 1. Remove the entire <section class="portfolios"> block
const portfolioStart = html.indexOf('<section class="portfolios"');
const servicesStart = html.indexOf('<section class="services">');

if (portfolioStart !== -1 && servicesStart !== -1) {
    // Delete everything from portfolios to services
    html = html.substring(0, portfolioStart) + html.substring(servicesStart);
}

// 2. Replace the services section styles and content
const newServicesHTML = `<section class="services pin-container">
    <div class="reveal-mask" style="padding: 0 5%;">
      <span class="eyebrow reveal-text">WHAT WE DO</span>
    </div>
    <div class="reveal-mask" style="padding: 0 5%;">
      <h2 class="section-heading reveal-text" style="margin-bottom: 0;">End-to-End Distribution,</h2>
    </div>
    ...
    <div class="scale-wrapper">
      ...
      <div class="scale-item" style="background-color: var(--primary-color);">
        <div class="scale-number" style="color: rgba(255,255,255,0.2);">05</div>
        <div class="scale-content">
          <div style="margin-bottom: 1rem;"><img src="assets/jockey_logo.png" alt="Jockey Logo" style="height: 40px; filter: brightness(0) invert(1);"></div>
          <h3 style="color: #fff;">Jockey<br>Distribution</h3>
          <p style="color: #eee;">Leo Group is the official Super Distributor for Jockey (Page Industries) across all of Kerala, managed directly under the Leo Group Team by specific managers dedicated solely to Jockey operations.</p>
        </div>
      </div>
      <div class="scale-item" style="background-color: var(--primary-color);">
        <div class="scale-number" style="color: rgba(255,255,255,0.2);">06</div>
        <div class="scale-content">
          <div style="margin-bottom: 1rem;"><img src="assets/ge_logo.png" alt="GE Logo" style="height: 40px; filter: brightness(0) invert(1);"></div>
          <h3 style="color: #fff;">Healthcare<br>Logistics</h3>
          <p style="color: #eee;">Managed by dedicated divisions like Leo Healthcare and Leo Logistics, corporate medical equipment distribution for GE HealthCare in Kerala is primarily managed by regional medical equipment dealers.</p>
        </div>
      </div>
    </div>
  </section>`;

const servicesStartRegex = /<section class="services">[\s\S]*?(?=<section class="legacy">)/;
html = html.replace(servicesStartRegex, newServicesHTML + '\n\n  ');

fs.writeFileSync('index.html', html, 'utf8');
console.log('Successfully updated index.html for horizontal scrolling layout.');
