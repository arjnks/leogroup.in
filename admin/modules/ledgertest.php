...
if(isset($_POST['sub_ledger']))
{
   if($_FILES['ledger_file']['name']==''){ $error[] = "Please upload the file"; }
	...
	if($error)
	{
		...
		session_write_close();
		header("location:  ./?p=ledgertest&tab=");
		exit();
	}
	else
	{
		...
		for($k=0;$k<=$q_count;$k++)
		{
			...
			if($k>$q_inc || $k==$q_count)
			{
				...
				{ 
					if($del_led==0)
					{
					$q_del = leo_mysql_query("DELETE FROM ledgertest WHERE date < DATE_SUB(NOW(), INTERVAL 30 DAY);"); $del_led = 1; 
					}
					...
					if($hh!=0)
					{*/
					$sql = "insert into ledgertest values ".$sql; 
...
if(isset($_POST['delete']))
{
	...
	$qry="delete from `ledgertest` where `id`=$id";
...
$cqry1="SELECT COUNT(*) FROM `ledgertest` ".$sub_qry; 
...
$total_pages = ceil($total_records / $count);
 ...
 $view="select * from `ledgertest` $sub_qry LIMIT $start_from , $count";
...
<?php
if(isset($_GET['cust_txt']) && $_GET['cust_txt']!=''){$find_cust=" where costomer_id='".$_GET['cust_txt']."'";}else{$find_cust='';}
$cqry1="SELECT COUNT(*) FROM `ledgertest` ".$find_cust; 
$cresult = @leo_mysql_query($cqry1); 
$row = mysql_fetch_row($cresult); 
$total_records = $row[0]; 
$total_pages = ceil($total_records / $count);
...
if(isset($_GET['cust_txt']) && $_GET['cust_txt']!=''){$find_cust=" where costomer_id='".$_GET['cust_txt']."'";}else{$find_cust='';}
$view="select * from `ledgertest` $find_cust LIMIT $start_from , $count";
$sql=leo_mysql_query($view);
$i=1;
while($db=leo_mysql_fetch_array($sql))
{
